...
def test_configuration_keys():
    assert ConfigurationKeys.allow_offline_tx_for_unknown_id == "AllowOfflineTxForUnknownId"
    assert ConfigurationKeys.authorization_cache_enabled == "AuthorizationCacheEnabled"
    assert ConfigurationKeys.authorize_remote_tx_requests == "AuthorizeRemoteTxRequests"
    assert ConfigurationKeys.blink_repeat == "BlinkRepeat"
    assert ConfigurationKeys.clock_aligned_data_interval == "ClockAlignedDataInterval"
    assert ConfigurationKeys.connection_time_out == "ConnectionTimeOut"
    assert ConfigurationKeys.connector_phase_rotation == "ConnectorPhaseRotation"
    assert ConfigurationKeys.connector_phase_rotation_max_length == "ConnectorPhaseRotationMaxLength"
    assert ConfigurationKeys.get_configuration_max_keys == "GetConfigurationMaxKeys"
    assert ConfigurationKeys.heartbeat_interval == "HeartbeatInterval"
    assert ConfigurationKeys.light_intensity == "LightIntensity"
    assert ConfigurationKeys.local_authorize_offline == "LocalAuthorizeOffline"
    assert ConfigurationKeys.local_pre_authorize == "LocalPreAuthorize"
    assert ConfigurationKeys.max_energy_on_invalid_id == "MaxEnergyOnInvalidId"
    assert ConfigurationKeys.meter_values_aligned_data == "MeterValuesAlignedData"
    assert ConfigurationKeys.meter_values_aligned_data_max_length == "MeterValuesAlignedDataMaxLength"
    assert ConfigurationKeys.meter_values_sampled_data == "MeterValuesSampledData"
    assert ConfigurationKeys.meter_values_sampled_data_max_length == "MeterValuesSampledDataMaxLength"
    assert ConfigurationKeys.meter_value_sample_interval == "MeterValueSampleInterval"
    assert ConfigurationKeys.minimum_status_duration == "MinimumStatusDuration"
    assert ConfigurationKeys.number_of_connectors == "NumberOfConnectors"
    assert ConfigurationKeys.reset_retries == "ResetRetries"
    assert ConfigurationKeys.stop_transaction_on_ev_side_disconnect == "StopTransactionOnEVSideDisconnect"
    assert ConfigurationKeys.stop_transaction_on_invalid_id == "StopTransactionOnInvalidId"
    assert ConfigurationKeys.stop_txn_aligned_data == "StopTxnAlignedData"
    assert ConfigurationKeys.stop_txn_aligned_data_max_length == "StopTxnAlignedDataMaxLength"
    assert ConfigurationKeys.stop_txn_sampled_data == "StopTxnSampledData"
    assert ConfigurationKeys.stop_txn_sampled_data_max_length == "StopTxnSampledDataMaxLength"
    assert ConfigurationKeys.supported_feature_profiles == "SupportedFeatureProfiles"
    assert ConfigurationKeys.supported_feature_profiles_max_length == "SupportedFeatureProfilesMaxLength"
    assert ConfigurationKeys.transaction_message_attempts == "TransactionMessageAttempts"
    assert ConfigurationKeys.transaction_message_retry_interval == "TransactionMessageRetryInterval"
    assert ConfigurationKeys.unlock_connector_on_ev_side_disconnect == "UnlockConnectorOnEVSideDisconnect"
    assert ConfigurationKeys.web_socket_ping_interval == "WebSocketPingInterval"
    assert ConfigurationKeys.local_auth_list_enabled == "LocalAuthListEnabled"
    assert ConfigurationKeys.local_auth_list_max_length == "LocalAuthListMaxLength"
    assert ConfigurationKeys.send_local_list_max_length == "SendLocalListMaxLength"
    assert ConfigurationKeys.reserve_connector_zero_supported == "ReserveConnectorZeroSupported"
    assert ConfigurationKeys.charge_profile_max_stack_level == "ChargeProfileMaxStackLevel"
    assert ConfigurationKeys.charging_schedule_allowed_charging_rate_unit == "ChargingScheduleAllowedChargingRateUnit"
    assert ConfigurationKeys.charging_schedule_max_periods == "ChargingScheduleMaxPeriods"
    assert ConfigurationKeys.connector_switch_3to1_phase_supported == "ConnectorSwitch3to1PhaseSupported"
    assert ConfigurationKeys.max_charging_profiles_installed == "MaxChargingProfilesInstalled"
