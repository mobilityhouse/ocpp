...
class CentralSystem(cp):
    ...
    def register_charger(self, cp: cp_cpo_v201.ChargePoint) -> asyncio.Queue:
    ...
    async def start_remote(self, cp_id, id: str, id_token: str, remote_start_id: int):
        """Starts a transaction remotely
        Not tested"""
        for cp, task in self._chargers.items():
            if cp.id == cp_id:
                await cp.send_request_start_transaction(id_token, remote_start_id)
                return 
        raise ValueError(f"Charger {id} not connected.")
            

    async def stop_remote(self, cp_id, id: str, transaction_id: str):
