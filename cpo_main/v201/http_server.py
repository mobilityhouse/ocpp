...
class HttpServer(web.View):
    ...
    async def authorize(request, id_tag, correct):
        data = {'id_token': ''}
        reference = data['id_token']
    ...
    @router.post("/config")
    @aiohttp_jinja2.template('config.html')
    async def set_variable(self):
        data = await self.post()
        cp_id = data['cp_id']
        set_variable_data = data['set_variable_data']
        cpo = self.app["cpo"]

        try:
            await cpo.set_variable(cp_id, set_variable_data)
    ...
    async def get_config(self):
        ...
        data = await self.post()
        cp_id = data['cp_id']
        get_variable_data = data['get_variable_data']
        cpo = self.app["cpo"]

        try:
            await cpo.get_configuration(cp_id, get_variable_data)
        except ValueError as e:
    ...
    async def availability(self):
        ...
        try:
            await cpo.change_availability(cp_id, operational_status)
        except ValueError as e:
    ...
    async def get_start_request(self):
        return {}
    ...
    async def get_stop_request(self):
        return
    ...
    async def get_composite_schedule(self):
        ...
        try:
            await cpo.get_schedule(cp_id, evse_id, duration, charging_rate_unit)
        except ValueError as e:
    ...
    async def send_local_list(self):
        ...
        try:
            await cpo.send_local_list(cp_id, version_number, local_authorization_list, update_type)
        except ValueError as e:
    ...
    async def reserve(self):
        ...
        expiry_date = data['expiry_date']
        id_token = {'id_token': data['id_token'], 'type': data['type']}

        cpo = self.app["cpo"]

        try:
            await cpo.reserve(cp_id, id, connector_id, expiry_date, id_token)
        except ValueError as e:
    ...
    async def clear_charging_profile(self):
        ...
        try:
            await cpo.clear_charging_profile(cp_id, charging_profile_id)
        except ValueError as e:
    ...
    async def unlock(self):
        ...
        cpo = self.app["cpo"]   
        try:
            await cpo.unlock_connector(cp_id, evse_id, connector_id)
        except ValueError as e:
