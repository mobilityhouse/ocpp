...
async def clear_tx_profile(cp):
    # Clear tx profile
    msg = call.ClearChargingProfilePayload(
        id=1111,
        connector_id=0
    )
    await cp.call(msg)    
...
async def send_change_config(cp):
    # Send change config
    msg = call.ChangeConfigurationPayload(
        key='SmartChargingEnabled',
        value='true'
    )
    await cp.call(msg)
...
async def send_trigger(cp):
    # Send trigger req
    msg = call.TriggerMessagePayload(
...
async def on_connect(websocket, path):
    ...
    if 'Authorization' in websocket.request_headers:
        logging.info("Got Authorization: {}".format(websocket.request_headers['Authorization']))
